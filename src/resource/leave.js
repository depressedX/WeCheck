...
export function getLeaveRequest(group_id) {
    return server.get('leave', {params: {group_id}})
}

//获取请假请求回馈
export function getAbsenceRequestFeedback(absenceId) {
    return server.get(`leave/${absenceId}/feedback`)
}
...
export function reportAbsence(group_id, result) {
    ...
    return server.post('group/leave', form)
}

//老师回复请假请求
export function responseToLeave(bundle) {
    return server.post('leave/response', qs.stringify(Object.assign(bundle)))
}
